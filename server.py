...
def getItem(item):
  ...
  if packaged.get(session['uid']) >= 2 and diff < 0:
    ...
    return render_template("results.html", resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), resultList4 = Markup(retVal4), diffOver = "${:.2f}".format(diff*-1), packagedconfirm = Markup(retVal6),)
  if diff >= 0:
...
def getItemsFromCategory(catg):
  ...
  if packaged.get(session['uid']) >= 2 and diff < 0:
    ...
    return render_template("category.html", resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), resultList4 = Markup(retVal4), diffOver = "${:.2f}".format(diff*-1), packagedconfirm = Markup(retVal6),)
  if diff >= 0:
    print("This 2")
    print(comboRet)
