...
def getSpecials():
  ...
  if packaged.get(session['uid']) == 2 and needAlert.get(session['uid']) == 1 and diff >= 0:
    ...
    needAlert[session['uid']] = 0
    return render_template("specials.html", comboBtn=Markup(comboStr), resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), surplus = "${:.2f}".format(diff), packagedconfirm = Markup(retVal6), resultTime = Markup(timeRet), lunchtime = Markup(retVal7))
  if packaged.get(session['uid']) > 2 and needAlert.get(session['uid']) == 1 and diff < 0:
    retVal6 = ''' <script> if (alert("2 packaged goods only! Please try another item.")) {
			}
			 </script> '''
    needAlert[session['uid']] = 0
    return render_template("specials.html", comboBtn=Markup(comboStr), resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), diffOver = "${:.2f}".format(diff*-1), packagedconfirm = Markup(retVal6), resultTime = Markup(timeRet), lunchtime = Markup(retVal7))
  if diff >= 0:
    return render_template("specials.html", comboBtn=Markup(comboStr), resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), resultList4 = Markup(retVal4), surplus = "${:.2f}".format(diff), resultTime = Markup(timeRet), lunchtime = Markup(retVal7))
  else:
    return render_template("specials.html", comboBtn=Markup(comboStr), resultList2 = Markup(retVal2), resultList3 = Markup(retVal3), resultList4 = Markup(retVal4), diffOver = "${:.2f}".format(diff*-1), resultTime = Markup(timeRet), lunchtime = Markup(retVal7))
...
def getFavorites():
  ...
  post1 = '''</h5></td><td class="button" style="padding-top:10px; padding-bottom:10px" onclick="javascript:window.location='/addItem/favorites/'''
  urlend = ''''">'''
  post2 = '''
...
def getItem(item):
  ...
  post_title = '''</h5></td>
      <td class="shop-item-price"><h5>$'''
  post1 = '''</h5></td><td class="button" style="padding-top:10px; padding-bottom:10px" onclick="javascript:window.location='/addItem/item/'''
  urlend = ''''">'''
  post2 = '''
...
def getItemsFromCategory(catg):
  ...
  post_title = '''</h5></td>
      <td class="shop-item-price"><h5>$'''
  post1 = '''</h5></td><td class="button" style="padding-top:10px; padding-bottom:10px" onclick="javascript:window.location='/addItem/'''
  urlend = ''''">'''
  post2 = '''
